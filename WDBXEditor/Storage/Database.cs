...
namespace WDBXEditor.Storage
{
    class Database
    {
        ...
        public static List<DBEntry> Entries { get; set; } = [];
        ...
        public static async Task<List<string>> LoadFiles(IEnumerable<string> filenames)
        {
            ...
            ConcurrentQueue<string> files = new(filenames.Distinct().OrderBy(x => x).ThenByDescending(x => Path.GetExtension(x)));
        ...
        {
            List<string> _errors = [];
            Queue<KeyValuePair<string, MemoryStream>> files = new(streams);
        ...
        {
            List<string> _errors = [];
            Queue<DBEntry> files = new(Entries);
