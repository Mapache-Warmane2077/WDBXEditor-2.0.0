...
{
    public partial class DBEntry : IDisposable
    {
        ...
        {
            // CÓDIGO ACTUALIZADO
            Data = new DataTable() { TableName = Tag, CaseSensitive = false, RemotingFormat = SerializationFormat.Xml };
            ...
            {
                Queue<TextWowEnum> languages = new(Enum.GetValues(typeof(TextWowEnum)).Cast<TextWowEnum>());
            ...
            Data.PrimaryKey = [Data.Columns[Key]];
        ...
        public void Attach()
        {
            ...
            string cachePath = Path.Combine(TEMP_FOLDER, Tag + ".cache");

            // Nos aseguramos de que el archivo exista antes de intentar leerlo
            if (File.Exists(cachePath))
            {
                // Creamos una nueva tabla en blanco
                DataTable loadedTable = new();

                // Leemos el esquema y los datos que guardamos previamente con WriteXml en Detach()
                loadedTable.ReadXml(cachePath);

                // Asignamos la tabla cargada a nuestra variable Data
                Data = loadedTable;
            }
        }
        ...
        public int[] GetPadding()
        {
            ...
            Dictionary<Type, int> bytecounts = new()
            {
                { typeof(byte), 1 },
                { typeof(short), 2 },
                { typeof(ushort), 2 },
            ...
            if (Header is WDC1 header)
            {
                ...
                foreach (var field in header.ColumnMeta)
                {
                    ...
                    bool isneeded = field.CompressionType >= WDBXEditor.Common.Constants.CompressionType.Sparse;

                    if (bytecounts.TryGetValue(type, out int value) && isneeded)
                    {
        ...
        public void ToSQLTable(string connectionstring)
        {
            ...
            using (StreamWriter csv = new(csvName))
                csv.Write(ToCSV());
            ...
            using (MySqlConnection connection = new(connectionstring))
            {
                ...
                using (MySqlCommand command = new(sb.ToString(), connection))
        ...
        {
            string[] columns = [.. Data.Columns.Cast<DataColumn>().Select(x => x.ColumnName)];
            ConcurrentBag<Dictionary<string, object>> Rows = [];
        ...
        public bool ImportSQL(UpdateMode mode, string connectionstring, string table, out string error, string columns = "*")
        {
            ...
            using (MySqlConnection connection = new(connectionstring))
            using (MySqlCommand command = new($"SELECT {columns} FROM `{table}`", connection))
        ...
        // IDE0060: Retirar el parámetro sin usar (importTable eliminado)
        private static bool ValidateMinMaxValues(out string error)
        {
            error = "";
        ...
        public void Dispose()
        {
            this.Data?.Dispose();
            this.Data = null;
            GC.SuppressFinalize(this); // CA1816: Evita llamadas redundantes al recolector de basura
        }

        // SYSLIB1045: Generador de expresiones regulares en tiempo de compilación para alto rendimiento
