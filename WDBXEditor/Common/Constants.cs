...
namespace WDBXEditor.Common
{
    public static class Constants
    {
        ...
        public static readonly Dictionary<TypeCode, byte> CommonDataTypes = new()
        {
            { TypeCode.String, 0 },
            { TypeCode.Int16, 1 },
            { TypeCode.UInt16, 1 },
        ...
        public static bool IsBuild(int build, Expansion expansion) => expansion switch
        {
            Expansion.Alpha => build <= (int)ExpansionFinalBuild.Alpha,
            Expansion.Beta => build > (int)ExpansionFinalBuild.Alpha && build <= (int)ExpansionFinalBuild.Beta,
            Expansion.Classic => build > (int)ExpansionFinalBuild.Beta && build <= (int)ExpansionFinalBuild.Classic,
            Expansion.TBC => build > (int)ExpansionFinalBuild.Classic && build <= (int)ExpansionFinalBuild.TBC,
            Expansion.WotLK => build > (int)ExpansionFinalBuild.TBC && build <= (int)ExpansionFinalBuild.WotLK,
            Expansion.Cata => build > (int)ExpansionFinalBuild.WotLK && build <= (int)ExpansionFinalBuild.Cata,
            Expansion.MoP => build > (int)ExpansionFinalBuild.Cata && build <= (int)ExpansionFinalBuild.MoP,
            Expansion.WoD => build > (int)ExpansionFinalBuild.MoP && build <= (int)ExpansionFinalBuild.WoD,
            Expansion.Legion => build > (int)ExpansionFinalBuild.WoD && build <= (int)ExpansionFinalBuild.Legion,
            Expansion.BfA => build > (int)ExpansionFinalBuild.Legion,
            _ => false
        };
        ...
        private static readonly SortedDictionary<int, string> Builds = new()
        {
            {3368, "Alpha 0.5.3"},
            {3494, "Alpha 0.5.5"},
            {3694, "Beta 0.7.0"},
        ...
        #region DB FileNames
        public static List<string> ClientDBFileNames { get; private set; } =
        [
        ...
        // Asegúrate de tener este using en la parte superior del archivo:
        // using System.Net.Http;
        // using System.IO;
        ...
        private static async Task LoadListFile()
        {
            ...
            if (!File.Exists(listfile) || (DateTime.Now - File.GetLastWriteTime(listfile)).TotalHours > 24)
            {
                try
                {
                    ...
                    // Reemplazamos HttpWebRequest por el moderno HttpClient
                    using HttpClient client = new();
                    // Asignamos el User-Agent para el rastreo del servidor
                    client.DefaultRequestHeaders.Add("User-Agent", "WDBXEditor/1.0 (+https://github.com/WowDevTools/WDBXEditor)");

                    // Obtenemos el flujo de datos de la web de forma asíncrona
                    using Stream webStream = await client.GetStreamAsync(url);
                    using FileStream fs = File.Create(listfile);
                    // Copiamos el archivo al disco duro de forma asíncrona (¡No congela el programa!)
                    await webStream.CopyToAsync(fs);
                    // Ya no es necesario usar req.Abort(), el bloque "using" limpia todo automáticamente
                }
                catch
                {
                    // Si falla la descarga y no hay archivo previo, detenemos el método
                    if (!File.Exists(listfile)) return;
                }
            }

            ClientDBFileNames = File.ReadAllLines(listfile).AsParallel()
                                    .Where(x => x.EndsWith("dbc", StringComparison.OrdinalIgnoreCase) || x.EndsWith("db2", StringComparison.OrdinalIgnoreCase))
