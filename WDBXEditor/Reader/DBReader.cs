...
namespace WDBXEditor.Reader
{
    public class DBReader
    {
        ...
        private readonly List<Tuple<int, short>> OffsetMap = [];
        ...
        public DBEntry Read(MemoryStream stream, string dbFile)
        {
            ...
            else if (header.IsTypeOf<WDB>())
            {
                ...
                using (BinaryReader dataReader = new(ms, Encoding.UTF8))
                {
                    ReadIntoTable(ref entry, dataReader, []);
                }
        ...
        public void ReadIntoTable(ref DBEntry entry, BinaryReader dbReader, Dictionary<int, string> StringTable)
        {
            ...
            TypeCode[] columnTypes = [.. entry.Data.Columns.Cast<DataColumn>().Select(x => Type.GetTypeCode(x.DataType))];
                    ...
                    switch (columnTypes[j])
                    {
                        ...
                        case TypeCode.String:
                            ...
                            else
                            {
                                ...
                                if (StringTable.TryGetValue(stindex, out string value))
                                {
                                    row.SetField(entry.Data.Columns[j], value);
                                }
                                else
                                {
        ...
        public void Write(DBEntry entry, string savepath)
        {
            ...
            StringTable st = new(entry.Header.ExtendedStringTable); //Preloads null byte(s)
            ...
            if (!entry.Header.IsTypeOf<WDC1>())
            {
                ...
                {
                    WriteIntoFile(entry, bw, [.. entry.GetUniqueRows()], ref st); //Insert unique rows
                }
                else
                {
                ...
                for (int j = 0; j < entry.Data.Columns.Count; j++)
                {
                    ...
                    switch (columnTypes[j])
                    {
                        ...
                        case TypeCode.String:
                            ...
                            break;
                        default:
                            throw new Exception($"Unknown TypeCode {columnTypes[j]}");
                    }
