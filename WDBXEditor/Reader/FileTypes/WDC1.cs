...
namespace WDBXEditor.Reader.FileTypes
{
    public class WDC1 : WDB6
    {
        ...
        // IDE0060: Se eliminó el parámetro 'pos' no utilizado
        public Dictionary<int, byte[]> ReadOffsetData(BinaryReader dbReader)
        {
            Dictionary<int, byte[]> CopyTable = [];
            List<Tuple<int, short>> offsetmap = [];
            Dictionary<int, OffsetDuplicate> firstindex = [];
            Dictionary<int, int> OffsetDuplicates = [];
            ...
            if (CopyTableSize > 0)
            {
                ...
                while (dbReader.BaseStream.Position < end)
                {
                    ...
                    // CA1864: TryAdd evita búsquedas dobles en el diccionario
                    Copies.TryAdd(idcopy, []);
            ...
            if (RelationshipDataSize > 0)
            {
                ...
                for (int i = 0; i < RelationShipData.Records; i++)
                {
                    ...
                    // CA1864: TryAdd evita la búsqueda doble
        ...
        public void LoadDefinitionSizes(DBEntry entry)
        {
            ...
            Dictionary<TypeCode, int> typeLookup = new()
            {
                { TypeCode.Byte, 1 },
                { TypeCode.SByte, 1 },
                { TypeCode.UInt16, 2 },
            ...
            columnSizes = [.. entry.Data.Columns.Cast<DataColumn>().Select(x => typeLookup[Type.GetTypeCode(x.DataType)])];
        }
        ...
        public void AddRelationshipColumn(DBEntry entry)
        {
            ...
            {
                DataColumn dataColumn = new("RelationshipData", typeof(uint));
        ...
        {
            object[] values = [.. Enumerable.Range(0, ColumnMeta[fieldIndex].ArraySize).Select(x => rowData.Dequeue())];
            isString = false;
        ...
        protected void RemoveBitLimits()
        {
            ...
            var skipType = new HashSet<CompressionType>([CompressionType.None, CompressionType.Sparse]);
