...
namespace WDBXEditor
{
    public partial class Main : Form
    {
        ...
        {
            string[] files = [((ToolStripMenuItem)sender).Tag.ToString()];
        ...
        private void GotoLine()
        {
            ...
            {
                // IDE0018: Variable 'id' declarada directamente en la llamada (out int id)
                if (int.TryParse(res, out int id)) //Ensure the result is an integer
                {
        ...
        private void Reload()
        {
            ...
            Task.Run(() => Database.LoadFiles([LoadedEntry.FilePath]))
            .ContinueWith(x =>
            {
                if (x.Result.Count > 0)
                    new ErrorReport(x.Result).ShowDialog(this);
        ...
        private void InsertLine()
        {
            ...
            {
                _ = advancedDataGridView.Columns[LoadedEntry.Key].Index;
        ...
        private void LoadRecentList()
        {
            ...
            foreach (var recent in Properties.Settings.Default.RecentFiles)
            {
                ...
                ToolStripMenuItem menuItem = new(recent, null, LoadRecentFilesToolStripMenuItem_Click)
                {
                    Tag = recent,
